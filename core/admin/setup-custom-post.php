...
//removes quick edit from maps,location type list
function mapsian_remove_quick_edit( $actions ) {
    global $post;
    if( $post->post_type == 'locations' or $post->post_type == 'maps') {
...
if (is_admin()) {
    add_filter('post_row_actions','mapsian_remove_quick_edit',10,2);
}



add_action( 'admin_menu', 'mapsian_menu_label_change' , 0 );

function mapsian_menu_label_change()
{
    global $menu, $submenu;

    $cpt_link = 'edit.php?post_type=maps'; // <-- adjust CPT
    $position = mapsian_array_search( $cpt_link, $menu );

    $menu[$position][0] = 'Mapsian'; // <-- adjust from here on
    $submenu[$cpt_link][5][0] = __('Maps','mapsian');   
}

function mapsian_array_search( $needle, $haystack ) 
{
    foreach( $haystack as $key => $value ) 
    {
        ...
        if( 
            ...
            OR ( 
                is_array( $value )
                && mapsian_array_search( $needle, $value ) !== false 
            )
        ) 
        {
            return $current_key;
...
function manage_locations_columns($column_name, $id) {
    ...
    //$location_meta = unserialize(base64_decode(get_post_meta($id, 'location_detail', true )));
    //this will check whether data is base64 encoded or not
    $get_location_detail = get_post_meta($id, 'location_detail', true );
